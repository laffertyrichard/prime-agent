...
Children receive incremented `RLM_DEPTH`, the inherited maximum depth, their own `RLM_SESSION_DIR`, and `RLM_SESSION_FILE` pointing to their conversation log. The default maximum depth is 1, so root sessions may create children and those children may not create grandchildren unless the limit is configured higher.
